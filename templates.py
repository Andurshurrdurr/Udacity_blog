...
# Gets the template directory for jinja
...
class Handler(webapp2.RequestHandler):
	...
	def validate(self):
		"""Validates user based on cookies.
...
# --------------------- Handlers for each page after this --------------------
...
class post(Handler):  # Shows info and options for specific blogpost
	def get(self):
		...
		if comment:  # Got comment
			...
			if user == entry.user.username:  # Check to see liker = poster
		...
		if entry.user.username == user:  # Check if user is Original Poster
			op = True
			print "User is Original Poster"
		else: 
			op = False
...
app = webapp2.WSGIApplication([	('/', mainpage),  # For routing
								('/login', login),
								('/logout', logout), 
								('/signup', signup), 
								('/submit', submit), 
